...
import { Heart, Sparkles, Cake, Gift, Music, Dog, Calendar, Flower2, GraduationCap, PartyPopper, X, ChevronLeft, ChevronRight, Camera } from 'lucide-react';
...
export default function VidhiBirthdayWebsite() {
  ...
  const openLightbox = (chapterId, photoIndex) => {
    // mark that this was opened interactively (not via the "Reveal Memories" button)
  ...
  // Prevent background scrolling when the lightbox (full-page viewer) is open
  useEffect(() => {
    try {
      if (lightboxOpen) {
        // store current overflow so we can restore it later
      ...
      // In environments without a DOM (tests), just ignore
    }
  ...
  const Lightbox = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-4">
        ...
        {/* Content Column: Image + Thumbnails */}
        <div className="w-full max-w-5xl mx-auto flex flex-col items-center">
          ...
          {/* Thumbnails strip inside lightbox for quick navigation */}
          <div className="mt-4 w-full overflow-x-auto py-2">
            <div className="flex items-center justify-start gap-2 px-2">
              {photos.map((p, idx) => (
                <img
                  ...
                  src={`/photos/${p}`}
                  alt={`Thumb ${idx + 1}`}
                  onClick={() => setLightboxImage({ chapterId: lightboxImage.chapterId, photoIndex: idx })}
                  className={`w-20 h-14 object-cover rounded-md cursor-pointer transition-transform duration-200 ${
                    idx === lightboxImage.photoIndex ? 'ring-4 ring-pink-500 scale-105' : 'opacity-70 hover:opacity-100 hover:scale-105'
                  }`}
                />
              ))}
            </div>
          ...
          {/* Counter */}
          <div className="mt-3 bg-white/10 text-white px-4 py-2 rounded-full">
